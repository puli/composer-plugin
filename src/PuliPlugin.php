...
class PuliPlugin implements PluginInterface, EventSubscriberInterface
{
    ...
    public function postInstall(CommandEvent $event)
    {
        ...
        $factoryManager = $puli->getFactoryManager();
        ...
        $this->regenerateFactoryClass($factoryManager, $io);
    ...
    private function regenerateFactoryClass(FactoryManager $factoryManager, IOInterface $io)
    {
        if (!$factoryManager->isFactoryClassAutoGenerated()) {
            return;
        }

        $io->write('<info>Regenerating the Puli factory class</info>');

        $factoryManager->autoGenerateFactoryClass();
    }
